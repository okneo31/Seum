...
TEST_CASE("양경로 v0.4a-5d: 이자 = 원금 x 이자율(BP) / 10000 (단리)", "[integration][both][v04a5d]") {
    auto b = run_both(
        U"가져오기(금융). 통화 마실.\n"
        U"계약 은행 {\n"
        U"  이자율: 450\n"
        U"  함수 이자(원금: 마실) -> 결과 { 돌려주기 마실(금액(원금) * 은행.이자율 / 10000). }\n"
        U"}\n"
        U"보여주기(은행.이자(마실(1000000))).");
    REQUIRE(b.tree == b.vm_);
    REQUIRE(b.tree.find(U"마실(45000)") != std::u32string::npos);
}

TEST_CASE("양경로 v0.4a-5d: 연체이자 = 연체금액 x 연체이자율 / 10000", "[integration][both][v04a5d]") {
    auto b = run_both(
        U"가져오기(금융). 통화 마실.\n"
        U"계약 은행 {\n"
        U"  연체이자율: 750\n"
        U"  함수 연체이자(연체금액: 마실) -> 결과 { 돌려주기 마실(금액(연체금액) * 은행.연체이자율 / 10000). }\n"
        U"}\n"
        U"보여주기(은행.연체이자(마실(300000))).");
    REQUIRE(b.tree == b.vm_);
    REQUIRE(b.tree.find(U"마실(22500)") != std::u32string::npos);
}

TEST_CASE("양경로 v0.4a-5d: 복리 = 반복으로 (1+r)^n 거듭제곱", "[integration][both][v04a5d]") {
    auto b = run_both(
        U"가져오기(금융). 통화 마실.\n"
        U"계약 은행 {\n"
        U"  이자율: 1000\n"
        U"  함수 복리(원금: 마실, 연수: 정수) -> 결과 {\n"
        U"    변수 잔액 = 금액(원금).\n"
        U"    반복 연수 번 { 변수 잔액 = 잔액 * (10000 + 은행.이자율) / 10000. }\n"
        U"    돌려주기 마실(잔액).\n"
        U"  }\n"
        U"}\n"
        U"보여주기(은행.복리(마실(1000000), 2)).");
    REQUIRE(b.tree == b.vm_);
    REQUIRE(b.tree.find(U"마실(1210000)") != std::u32string::npos);
}

TEST_CASE("양경로 v0.4a-5d: 대출 복식부기 — 이자도 미수이자/이자수익 양쪽 기입", "[integration][both][v04a5d]") {
    auto b = run_both(
        U"가져오기(금융). 통화 마실.\n"
        U"계약 은행 {\n"
        U"  자산 전체계좌합: 마실\n"
        ...
        U"  이자율: 450\n"
        U"  함수 이자(원금: 마실) -> 결과 { 돌려주기 마실(금액(원금) * 은행.이자율 / 10000). }\n"
        U"  함수 대출(원금: 마실) 받아서 -> 전체계좌합 {\n"
        U"    변수 이자분 = 은행.이자(원금).\n"
        U"    은행.대출채권 += 원금. 은행.미수이자 += 이자분. 은행.이자수익 += 이자분.\n"
        U"  }\n"
        U"}\n"
        U"은행.대출(마실(1000000)).\n"
        U"보여주기(은행.미수이자). 보여주기(은행.이자수익).");
    REQUIRE(b.tree == b.vm_);
    REQUIRE(b.tree.find(U"마실(45000)") != std::u32string::npos);
}

// === v0.4a-5e: 대출 3 규제 브레이크 (예대율·지급준비율·BIS) — 양 경로 ===

static const char32_t* 은행_브레이크 =
    U"가져오기(금융). 통화 마실.\n"
    U"계약 은행 {\n"
    U"  자산 전체계좌합: 마실\n"
    U"  자산 대출채권: 마실\n"
    ...
    U"  예대율상한: 10000\n"
    U"  함수 승인가능(원금: 마실) -> 결과 {\n"
    U"    변수 새예금 = 금액(은행.전체계좌합) + 금액(원금).\n"
    U"    변수 새대출 = 금액(은행.대출채권) + 금액(원금).\n"
    U"    변수 지준통과 = 금액(은행.본원통화) * 10000 >= 새예금 * 은행.지급준비율.\n"
    U"    변수 자본통과 = 금액(은행.자기자본) * 10000 >= 새대출 * 은행.자본비율하한.\n"
    U"    변수 예대통과 = 새대출 * 10000 <= 새예금 * 은행.예대율상한.\n"
    U"    만약 (지준통과 && 자본통과 && 예대통과) 이면 { 돌려주기 참. } 아니면 { 돌려주기 거짓. }\n"
    U"  }\n"
    U"  함수 대출(원금: 마실) -> 결과 {\n"
    U"    만약 (은행.승인가능(원금)) 이면 { 은행.전체계좌합 += 원금. 은행.대출채권 += 원금. 돌려주기 \"승인\". }\n"
    U"    아니면 { 돌려주기 \"거부\". }\n"
    U"  }\n"
    U"}\n";

TEST_CASE("양경로 v0.4a-5e: 3 브레이크 게이트 — 승인 후 BIS 거부", "[integration][both][v04a5e]") {
    auto b = run_both(std::u32string(은행_브레이크) +
        U"보여주기(은행.대출(마실(2500000))).\n"
        U"보여주기(은행.대출(마실(2500000))).\n"
        U"보여주기(은행.대출(마실(2500000))).\n"
        U"보여주기(은행.대출채권).");
    REQUIRE(b.tree == b.vm_);
    REQUIRE(b.tree.find(U"승인") != std::u32string::npos);
    REQUIRE(b.tree.find(U"거부") != std::u32string::npos);
    REQUIRE(b.tree.find(U"마실(5000000)") != std::u32string::npos);  // 승인 2건만, 거부분 미반영
}

TEST_CASE("양경로 v0.4a-5e: 대출 거부 — 지급준비율 부족", "[integration][both][v04a5e]") {
    auto b = run_both(
        U"가져오기(금융). 통화 마실.\n"
        U"계약 은행 {\n"
        U"  자산 전체계좌합: 마실\n"
        ...
        U"  }\n"
        U"}\n"
        U"보여주기(은행.대출(마실(5000000))).\n"
        U"보여주기(은행.전체계좌합).");
    REQUIRE(b.tree == b.vm_);
    REQUIRE(b.tree.find(U"거부") != std::u32string::npos);
    REQUIRE(b.tree.find(U"마실(0)") != std::u32string::npos);  // 거부 → 장부 불변
}

// === v0.4a-5f: 자유은행 — 담보 기반 자동 청산 (SATSTD-style) ===

TEST_CASE("양경로 v0.4a-5f: 자유은행 자동 청산 — 가격 폭락 시 결정적 집행", "[integration][both][v04a5f]") {
    auto b = run_both(
        U"가져오기(금융). 통화 마실 = BTC / 100000.\n"
        U"계약 자유은행 {\n"
        U"  자산 담보BTC: BTC\n"
        U"  자산 발행마실: 마실\n"
        U"  BTC시세: 100000\n"
        U"  청산비율: 15000\n"
        U"  함수 담보가치() -> 결과 { 돌려주기 금액(자유은행.담보BTC) * 자유은행.BTC시세. }\n"
        U"  함수 발행(담보: BTC, 발행양: 마실) -> 결과 {\n"
        U"    변수 새가치 = (금액(자유은행.담보BTC) + 금액(담보)) * 자유은행.BTC시세.\n"
        U"    변수 새발행 = 금액(자유은행.발행마실) + 금액(발행양).\n"
        U"    만약 (새가치 * 10000 >= 새발행 * 자유은행.청산비율) 이면 {\n"
        U"      자유은행.담보BTC += 담보. 자유은행.발행마실 += 발행양. 돌려주기 \"발행\".\n"
        U"    } 아니면 { 돌려주기 \"발행거부\". }\n"
        U"  }\n"
        U"  함수 가격갱신(새시세: 정수) -> 없음 { 자유은행.BTC시세 = 새시세. }\n"
        U"  함수 청산실행() -> 결과 {\n"
        U"    만약 (자유은행.담보가치() * 10000 < 금액(자유은행.발행마실) * 자유은행.청산비율) 이면 {\n"
        U"      자유은행.담보BTC -= 자유은행.담보BTC. 자유은행.발행마실 -= 자유은행.발행마실.\n"
        U"      돌려주기 \"청산완료\".\n"
        U"    } 아니면 { 돌려주기 \"청산조건 미충족\". }\n"
        U"  }\n"
        U"}\n"
        U"자유은행.발행(BTC(2), 마실(100000)).\n"
        U"자유은행.가격갱신(60000).\n"
        U"보여주기(자유은행.청산실행()).\n"
        U"보여주기(자유은행.담보BTC). 보여주기(자유은행.발행마실).");
    REQUIRE(b.tree == b.vm_);
    REQUIRE(b.tree.find(U"청산완료") != std::u32string::npos);
    REQUIRE(b.tree.find(U"BTC(0)") != std::u32string::npos);
    REQUIRE(b.tree.find(U"마실(0)") != std::u32string::npos);
}

TEST_CASE("양경로 v0.4a-5f: 자유은행 발행 거부 — 담보 부족", "[integration][both][v04a5f]") {
    auto b = run_both(
        U"가져오기(금융). 통화 마실 = BTC / 100000.\n"
        U"계약 자유은행 {\n"
        U"  자산 담보BTC: BTC\n"
        ...
        U"  }\n"
        U"}\n"
        U"보여주기(자유은행.발행(BTC(1), 마실(100000))).");
    REQUIRE(b.tree == b.vm_);
    REQUIRE(b.tree.find(U"발행거부") != std::u32string::npos);
}
